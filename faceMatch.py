...
def initFaceRecog():
    ...
    # Init Model
    node_cascade = cv2.CascadeClassifier('/home/nodeKeeper/script/cascade.xml')
...
def cam_photo(camera):
    ...
    camera.capture('/home/nodeKeeper/image_test/' + '%s' %(timecam))
    camera.stop_preview()
    return(timecam)
    
def analyse_photo(filename, node_cascade):
    img = cv2.imread('/home/nodeKeeper/image_test/' + filename)
...
def create_photo_analyse(filename, node_cascade):
    ...
    plt.savefig('/home/nodeKeeper/image_analyse/' + filename)
